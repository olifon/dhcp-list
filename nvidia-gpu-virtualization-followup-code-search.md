...
But symbol search surfaced something repo-level search could not: **one half of approach C has already been built and published.** `0xf4b1/bsod-kernel-fuzzing` ships a QEMU device model at `bsod-fakedev/qemu/hw/pci-replay/pci-nvidia.c` that fabricates a synthetic NVIDIA PCI device which the **unmodified NVIDIA driver binds to** — built at TU Berlin for driver fuzzing, and it found three real CVEs. It replays captured MMIO traces rather than implementing GSP semantics, so no actual GPU work executes. That is precisely the boundary: *getting a stock NVIDIA driver to attach to a fake device in a VMM is done and published; making that fake device functional is not.*
...
|---|---|---|
| **GitHub-wide code search never ran** | The single remaining hole. Sourcegraph demonstrably misses the 0–100-star band — the exact band where a 3-commit abandoned attempt would live. My null covers *indexed* code, not *all* code. | An authenticated `gh search code` (or a session whose GitHub scope permits global search) over the brief's symbol list, filtered `-user:NVIDIA -path:open-gpu-kernel-modules` and by `language:rust|go|python|c++|zig`. Perhaps 20 minutes with a token. |
